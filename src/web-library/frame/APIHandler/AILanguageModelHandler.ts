...
class AILanguageModelHandler {
  ...
  async #buildPrompt (
    ...
    while (true) {
      ...
      const tokenCount = await AILlmSession.countTokens(sessionId, prompt, props, {})
  ...
  #handleCreate = async (channel: IPCInflightChannel) => {
    return await APIHelper.handleStandardCreatePreflight(channel, AIModelType.Text, AICapabilityPromptType.LanguageModel, async (
      ...
      const tokensSoFar = await AILlmSession.countTokens(
        sessionId,
        await this.#buildPrompt(sessionId, manifest, props, systemPrompt, initialPrompts, []),
        props,
        {}
      )
  ...
  #handlePrompt = async (channel: IPCInflightChannel) => {
    return await APIHelper.handleStandardPromptPreflight(channel, AIModelType.Text, async (
      ...
      const prompt = await this.#buildPrompt(sessionId, manifest, props, systemPrompt, initialPrompts, messages)
      if (channel.abortSignal?.aborted) { throw new Error(kModelPromptAborted) }
