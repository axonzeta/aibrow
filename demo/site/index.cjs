...
logTask('Checking for window.ai...', (log) => {
  log(!!window.ai)
})
...
$go.addEventListener('click', async () => {
  ...
  try {
...

    {
      ...
      switch ((await window.ai.assistant.capabilities(sessionOpts)).available) {
        case 'readily':
          logRes(true)
          break
        case 'after-download':
    ...
    if (session) {
      ...
      session = await logTask('Creating session...', async (log) => {
        const res = await window.ai.assistant.create(sessionOpts)
        log(true)
        return res
      })
    }
